...
window.addEventListener('DOMContentLoaded', (event) => {
  (function ($, window, document, undefined) {
    ...
    $("#open-multiple-choose-modal").on("click", function (e) {
      ...
      startActiveObject(circle);
      canvas.add(circle);
      circle.on("mouseup", function (e) {
        if (e.button === 3) {
          selectedObject = circle;
          showPopUpMenuQuiz(circle);
        }
      });
    ...
    canvas.on("object:added", function (obj) {
      if (obj.target && !obj.target.isBackground) {
        ...
        if (obj.target.name.startsWith("quiz-multipleObj")) {
          obj.target.setControlsVisibility({
            tl: true,
            tr: true,
            bl: true,
...
document.getElementById('sharpnessButton').addEventListener('click', applySharpness);
        ...
        document.getElementById('showTextEditor').addEventListener('click', function () {
           var textEditorContainer = document.getElementById('textEditorContainer');
           textEditorContainer.style.display = (textEditorContainer.style.display === 'none' || textEditorContainer.style.display === '') ? 'block' : 'none';
       });
